...
class Checkers:
    ...
    def get_best_move(self, moves: list[Move]) -> Move | None:
        ...
        def threatened_by(piece: Piece) -> list[Piece]:
            """
            :return: the list of pieces that can jump over the given piece
            """
            result: list[Piece] = []

            valid_moves = self.get_all_valid_moves(piece.side.get_enemy())
            for valid_move in valid_moves:
                result += valid_move.jumped_pieces
        ...
        # Score moves
        scores: dict[moves: Move, score: float] = {}
        for move in moves:
            score: float = 0
